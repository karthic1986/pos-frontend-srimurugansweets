...
const REPORT_ROUTES = ["/deliveryReport", "/nondeliveryReport", "/productReport", "/customerReport", "/datewiseReport"];

const MainSidebar = () => {
  const { pathname } = useLocation();
  const navLinkClass = (path) => `nav-link${pathname === path ? " active" : ""}`;
  const isReportRoute = REPORT_ROUTES.includes(pathname);
      ...
      <div className="sidebar">
        ...
        <nav className="mt-2">
          <ul className="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li className={`nav-item${reportsOpen ? " menu-open" : ""}`}>
              <a href="#" onClick={toggleReports} className={`nav-link${isReportRoute ? " active" : ""}`}>
